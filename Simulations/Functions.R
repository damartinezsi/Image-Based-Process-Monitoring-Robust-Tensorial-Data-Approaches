...
run_simulation <- function(i, 
                           delta = 0, #Controla delta5 y delta6
                           n_samples = 60, 
                           prop_in_control = 0.9,
                           # Para pixeles atípicos
                           noise_frac = 0.0,  # % de entradas atipicas (pixeles)
                           noise_sigma2 = 1,   # varianza 
                           # Para regiones atípicas
                           region_enable = FALSE,
                           region_mode = "center", #c("center","edge_tl")
                           region_size = 6,
                           region_sigma2 = 0.1,
                           clip_0255 = FALSE, 
                           # Parametros Yan&Paynabar
                           d1delta=0,
                           d2delta=0,
                           d3delta=0,
                           d4delta=0
  ...
  # Generar datos
  # Generar muestras en control (todo en 0)
  ...
  # ===== Inyección de ruido en x% de entradas por imagen =====
  if (n_out_control > 0 && noise_frac > 0) {
    sd_noise <- sqrt(noise_sigma2)
    
    for (ii in 1:n_out_control) {
      # vectorizar la imagen ii
      img_vec <- as.vector(tensor_out_control[ii,,,])
      
      # seleccionar índices válidos 
      valid_idx <- which(!is.na(img_vec))
      k <- max(1, round(length(valid_idx) * noise_frac))
      
      # muestreo aleatorio de posiciones a perturbar
      idx_noise <- sample(valid_idx, k)
      
      # sumar ruido ~ N(mu, sigma2)
      img_vec[idx_noise] <- img_vec[idx_noise] + rnorm(k, mean = delta, sd = sd_noise)
      
      # rearmar al arreglo 3D original
      tensor_out_control[ii,,,] <- array(img_vec, dim = img_shape)
    }
  }
  
  # ================== Inyección regional fija (10x10) ====================
  if (region_enable && n_out_control > 0) {
    H <- img_shape[1]; W <- img_shape[2]; C <- img_shape[3]
    s <- region_size
    region_sd <- sqrt(region_sigma2)
    mu <- delta  # media del ruido regional
    ...
    # coordenadas de la ventana s×s centrada en (cx0, cy0)
    x_min0 <- max(1, round(cx0) - floor(s/2)); x_max0 <- min(W, x_min0 + s - 1)
    y_min0 <- max(1, round(cy0) - floor(s/2)); y_max0 <- min(H, y_min0 + s - 1)
    
    # matriz de incrementos (mismo incremento por píxel para todos los canales)
    h_win <- y_max0 - y_min0 + 1
    w_win <- x_max0 - x_min0 + 1
    k <- h_win * w_win
    
    for (ii in 1:n_out_control) {
      img <- tensor_out_control[ii,,,]
      if (k > 0) {
        inc <- matrix(rnorm(k, mean = mu, sd = region_sd), nrow = h_win)
        for (ch in 1:C) {
          img[y_min0:y_max0, x_min0:x_max0, ch] <- img[y_min0:y_max0, x_min0:x_max0, ch] + inc
        }
        if (clip_0255) {
          img[y_min0:y_max0, x_min0:x_max0, ] <- pmin(pmax(img[y_min0:y_max0, x_min0:x_max0, ], 0), 255)
        }
      }
      tensor_out_control[ii,,,] <- img
    }
  }
  # ======================================================================
...
if (!is.null(MACRO_result)) {
  ...
  trod_mcd <- tryCatch(CovMcd(theta_TROD, alpha = 0.8011500),
  ...
  MPCA_mcd <- tryCatch(CovMcd(theta_MPCA, alpha = 0.7093500),
                       error = function(e) NULL)
  ...
  MACRO_mcd <- tryCatch(CovMcd(theta_MACRO, alpha = 0.7341000),
                        error = function(e) NULL)
...
Best_alpha <- function(i, delta = 0, n_samples = 60, prop_in_control = 0.9,
